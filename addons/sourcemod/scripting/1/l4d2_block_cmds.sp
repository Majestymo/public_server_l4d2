...
{
	g_hBlockCmds	= CreateConVar("blockcmds_list", "sm_pw;sm_rpg;sm_buy;sm_boom;sm_explode;sm_vip;sm_help", "使用';'号分隔要禁用的命令.");
...
void PunishType(int client)
{
	...
	{
		case 0:
			PrintToChat(client, "\x04[提示]\x03%N\x05,请勿在本服使用rpg指令.", client);
		case 1:
		{
			CheckIdleBot(client);
			PrintToChat(client, "\x04[提示]\x03%N\x05,由于你被rpg服荼毒已深,已被处死.", client);
		}
		case 2:
			KickClient(client, "[提示]由于你被rpg服荼毒已深,已被踢出服务器.");
		case 3:
			BanClient(client, g_hBanTime.IntValue, BANFLAG_AUTO, "Banned", "[提示]由于你被rpg服荼毒已深,已被封禁.");
	}
}
