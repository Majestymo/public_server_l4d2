			...
			if (IsValidSurvivor(g_iTargetSurvivor))
			{
				...
				if (!IsPlayerVisibleTo(fSpawnPos) && IsOnValidMesh(fSpawnPos) && !IsPlayerStuck(fSpawnPos))
				{
					...
					for (int count = 0; count < g_iSurvivorNum; count++)
					{
						...
						fSurvivorPos[2] -= 60.0;
						if (L4D2_NavAreaBuildPath(fSpawnPos, fSurvivorPos, g_fSpawnDistanceMax + 250.0, false, false, TEAM_INFECTED, false) && GetVectorDistance(fSurvivorPos, fSpawnPos) > g_fSpawnDistanceMin)
						{
							int iZombieClass = IsBotTypeNeeded();
							if (iZombieClass > 0)
							{
				...
				if (IsOnValidMesh(fSpawnPos) && !IsPlayerStuck(fSpawnPos) && !IsPlayerVisibleTo(fSpawnPos))
				{
					for (int count = 0; count < g_iSurvivorNum; count++)
					{
						...
						if (IsClientInGame(index))
						{
							GetClientEyePosition(index, fSurvivorPos);
							fSurvivorPos[2] -= 60.0;
							if (L4D2_NavAreaBuildPath(fSpawnPos, fSurvivorPos, g_fTeleportDistance, false, false, TEAM_INFECTED, false) && GetVectorDistance(fSelfEyePos, fSpawnPos) > g_fTeleportDistance && GetVectorDistance(fSelfEyePos, fSpawnPos) > g_fSpawnDistanceMin)
							{
