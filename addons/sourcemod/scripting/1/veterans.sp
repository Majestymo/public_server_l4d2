	...
	{	
		...
		if (GetConVarBool(cvar_excludeGroupMember) && player[client].isGroupMember)
		{
			if(GetConVarBool(cvar_excludeGroupMemberPlay))
				CPrintToChatAll("{default}[{green}玩家时长检测{default}] 玩家 {blue}%N {{default}在本服务器游戏时长{green}不达标{default}，是服务器组玩家，允许正常游玩", client);
			else
				CPrintToChatAll("{default}[{green}玩家时长检测{default}] 玩家 {blue}%N {{default}在本服务器游戏时长{green}不达标{default}，是服务器组玩家，允许旁观不允许游玩", client);
			//PrintToServer("VeteransOnly: Excluded for being a group member");
			return;
		}
		else{
			CPrintToChatAll("{default}[{green}玩家时长检测{default}] 玩家 {blue}%N {default}在本服务器游戏时长{green}不达标{default}，且不为服务器组玩家，已经自动请去休息.",client);
		}		
	}
